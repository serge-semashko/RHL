...
{$R *.DFM}
   ...
   begin
     result := trunc((v_cur - 0.01037423975618015)/ 0.0000760582458516763);
   end;
   function V_revert(v_ref:double;d_cur:dword) : double;
   begin
     result :=0.0000760582458516763 * d_cur - 0.0057423975618;
   end;
...
border_high := v_convert(v_ref,topspn.Value/10000.0);
border_low := v_convert(v_ref,bottomspn.Value/10000.0);
if border_low<1 then border_low:=1;
if border_high<1 then border_high:=1;
if border_high<border_low then border_high:=border_low;

curControl := border_low-1;
  ...
  while CycBox.Checked do begin
   ...
   for il1:= prevGRindex to vindex do begin
    ...
    if (now-start_step)*24*3600>step_len then begin
       curControl := curControl + dstep*stepspn.Value;
       if curControl > border_high then begin
          dstep := -1;
       end else if curControl < border_low then begin
          dstep := 1;
       end;
...
begin
  ...
  if border_high > border_low then dstep:=1 else dstep:=-1;
      dacseries.Clear;
  while (curControl < 65000) and (startdacbtn.Caption = 'Stop') do begin
    if dstep <0 then
...
//    dacseries.AddXY(curControl,curVoltage);
    ...
    curControl := curControl + dstep*70;
  end;
    delay(3);
    daqGetdata;
    if startdacbtn.Caption <> 'Stop' then begin
      curControl := curControl+trunc((Topspn.Value*1.0-curVoltage*10000)/0.75);
      setDPotential(border_high);
      delay(3);
      daqGetdata;
    end;
    startdacbtn.Caption := 'Start';
