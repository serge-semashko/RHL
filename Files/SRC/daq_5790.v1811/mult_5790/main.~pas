...
begin
    ...
    url := 'http://192.168.0.12:8282/setabs=' + s1;
...
//          cntval :=10;
        ...
        if (ibsta and $8000)<>0
          then dacval := -1
            ...
            decimalseparator := '.';
            if not TextToFloat(PChar(resstr), dacval, fvExtended) then dacval := -1;
          end;

        curmtime := timegettime();


        if (dacval < 0) then begin
...
//          '   Progress: '+IntToStr(trunc((now-SweepStartTime)*100/sweep_duration))+'%' ;
        ...
        CurCount.Text := format('%f', [ Counter_per_sec]);
  end;
end;
