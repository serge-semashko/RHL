...
//        writeprotocol('24');
          resstr := rdstr + ' ' + format(' %.4d %.6d %.1d %.8d %.8d %.3d', [cntval, curControl, dstep, vindex + 1, stepStartIndex, deltaCounterValue]) + ' ' + intToStr(trunc((now - prevMtime))) + ' ' + StringReplace(floattostr(now), ',', '.', [rfReplaceAll, rfIgnoreCase]);
//        writeprotocol('14');
        prevread := now();
//        writeprotocol('rdstr: ' + rdstr);
...
//         writetimelog( ' GettingData = '+IntToStr(integer(GettingData))+' vindex='+IntToStr(vindex)+#13);
        ...
        if GettingData then begin
                resstr := format('%.4f %.4d %.6d %.1d %.8d %.8d %.3d',
