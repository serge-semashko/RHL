...
//       while (now-start_Step)*24*3600<0.4 do application.processmessages;
            ...
            if  spectrechart.RightAxis.Maximum < CounterStep*1.05 then spectrechart.RightAxis.Maximum := CounterStep*1.05;
            i1 := 0;
            ...
            if i1 >=  psreserv.Count
               then begin
                psreserv.AddXy(curTarget, CounterStep);
                writeTimelog(format('create spectre curtarget=%.1f Cnt=%d' ,[curtarget, CounterStep] )+#10);
            end;
