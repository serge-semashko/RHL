...
//         fg,gb,b,bvvn
    ...
    var
        tmpstr : string;
    begin
      tmpstr := Format('Region: %d, Sweep= %d, SweepMode= %s',[ zqry1.RecordCount, Stage, sweepmodes[dstep+2]])+#10;
      WriteLog(DataDirName+IntToStr(stage),tmpstr);
      WriteLog(DataDirName+IntToStr(stage),headerstr);
      WriteLog(DataDirName+IntToStr(stage),#10);
    ...
    begin
       ...
       writetimelog(DataDirName + 'range.txt', tmpstr);
        ...
        if daq_mode = 1 then begin
            if dstep < 0 then
                showerrorRange;
            if round(Curtarget) < border_low then
                showerrorRange;
        end;
        ...
        if (dstep > 0) then begin
            if round(Curtarget) < border_low then
                showerrorRange;
            if round(Curtarget) > border_high then begin
                zqry1.MoveBy(1);
                setrangeparams;
...
begin
    ...
    Writelog(DataFileName, tmpstr+#10);
    Headerstr := '';
    ...
    WriteLog(DataFileName, headerstr+#10);
...
//       while (now-start_Step)*24*3600<0.4 do application.processmessages;
    ...
    //   writetimelog(rdstr+#10);
        ...
        else if (now - CorrectionTime) * 24 * 3600 > 5 then begin
