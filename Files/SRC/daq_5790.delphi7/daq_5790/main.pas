...
uses
  ...
  Synautil, Grids, DB, ADODB, DBGrids, ZAbstractRODataset,
...
begin
  ...
  if counter_and_GPIB_ready then
    exit;
  ...
  begin
    Beep;
    iConfirm := MessageDlg('OPEN_COM Error Code:' + IntToStr(iRet) + #13 + IGetErrorString(iRet) + #13 + 'Quit this demo?', mtConfirmation, [mbYes, mbNo], 0);
...
begin
  showmessage( dataset.FieldByName('num').AsString  +' '+
               dataset.FieldByName('lowv').AsString+' '+
               dataset.FieldByName('highv').AsString+' '+
               dataset.FieldByName('exposition').AsString+' ');
end;
