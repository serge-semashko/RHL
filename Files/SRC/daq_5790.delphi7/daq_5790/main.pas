...
{$R *.DFM}
 ...
 var
  s1,url: string;
 begin
     if abs(oldControl-u)>(0.01*$FFFFF/5)
        then curcontrol:= oldcontrol+trunc((0.01*$FFFFF/5)*(oldControl-u)/abs(oldControl-u));
     curControl := u;
...
//       while (now-start_Step)*24*3600<0.4 do application.processmessages;
    ...
    //   writetimelog(rdstr+#10);
       ...
       while (now-start_Step)*24*3600<deadspn.Value-2 do application.processmessages;
